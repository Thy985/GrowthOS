...
export const loadGoals = createAsyncThunk('goal/loadGoals', async () => {
  try {
    logger.info('加载目标数据');
    const goals = (secureStorage.getItem<Goal[]>('growth-goals') || []) as Goal[];
...
export const addGoal = createAsyncThunk(
  'goal/addGoal',
  async (goal: Omit<Goal, 'id' | 'currentValue' | 'status' | 'createdAt' | 'updatedAt'>) => {
    try {
...
export const updateGoal = createAsyncThunk(
  'goal/updateGoal',
  async (goal: Partial<Goal> & { id: string }) => {
    try {
      logger.info('更新目标', { goalId: goal.id });
      const goals = (secureStorage.getItem<Goal[]>('growth-goals') || []) as Goal[];

      const updatedGoals = goals.map((g) =>
        g.id === goal.id ? { ...g, ...goal, updatedAt: new Date().toISOString() } : g,
      );
...
export const incrementGoalProgress = createAsyncThunk(
  ...
  async ({ goalId, value }: { goalId: string; value: number }) => {
    try {
      ...
      const updatedGoals = goals.map((g: Goal) => {
        if (g.id === goalId) {
          ...
          let newStatus: Goal['status'] = g.status;
      ...
      const updatedGoal = updatedGoals.find((g: Goal) => g.id === goalId);
...
const goalSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        incrementGoalProgress.fulfilled,
        (state, action: PayloadAction<Goal | undefined>) => {
          state.isLoading = false;
          if (action.payload) {
            const index = state.goals.findIndex((g) => g.id === action.payload?.id);
            if (index !== -1) {
