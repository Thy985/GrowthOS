...
const Goals = () => {
  ...
  const handleAddGoal = useCallback(
    (e: React.FormEvent) => {
      ...
      setFormData({ title: '', description: '', targetValue: '', startDate: '', endDate: '' });
  ...
  const handleIncrementProgress = useCallback(
    (goalId: string) => {
      const value = prompt(t('goals.promptProgress', '请输入要增加的进度值：'));
  ...
  return (
    <ErrorBoundary>
      <div className="goals-page">
        <h1 className="page-title">{t('goals.title', '目标管理')}</h1>

        {error && <div className="error-message">{error}</div>}

        <div className="goals-header">
          <button className="btn btn-primary" onClick={() => setShowAddForm(!showAddForm)}>
            {showAddForm ? t('common.cancel', '取消') : t('goals.addGoal', '添加目标')}
          </button>
        </div>

        {showAddForm && (
          <div className="goal-form card">
            <h2 className="text-xl font-semibold mb-4">{t('goals.addNewGoal', '添加新目标')}</h2>
            <form onSubmit={handleAddGoal}>
              <div className="form-group">
                <label className="form-label">{t('goals.goalTitle', '目标标题')}</label>
                <input
                  type="text"
                  name="title"
                  className={`input ${formErrors.title ? 'border-error' : ''}`}
                  placeholder={t('goals.titlePlaceholder', '例如：每天学习1小时')}
              ...
              <div className="form-group">
                <label className="form-label">{t('goals.goalDescription', '目标描述')}</label>
                <textarea
                  name="description"
                  className="input"
                  rows={3}
                  placeholder={t('goals.descriptionPlaceholder', '描述你的目标...')}
                  value={formData.description}
                  onChange={handleChange}
                ></textarea>
              </div>
              <div className="form-group">
                <label className="form-label">{t('goals.targetValue', '目标值')}</label>
                <input
                  type="number"
                  name="targetValue"
                  className={`input ${formErrors.targetValue ? 'border-error' : ''}`}
                  placeholder={t('goals.targetValuePlaceholder', '例如：30')}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="form-group">
                  <label className="form-label">{t('goals.startDate', '开始日期')}</label>
                  <input
                    type="date"
                    name="startDate"
                ...
                <div className="form-group">
                  <label className="form-label">{t('goals.endDate', '结束日期')}</label>
                  <input
                    type="date"
                    name="endDate"
              ...
              <button type="submit" className="btn btn-primary w-full mt-4" disabled={isLoading}>
                {isLoading ? t('goals.adding', '添加中...') : t('goals.addGoal', '添加目标')}
              </button>
            </form>
          </div>
        )}
        ...
        {showEditForm && currentGoal && (
          <div className="goal-form card">
            <h2 className="text-xl font-semibold mb-4">{t('goals.editGoal', '编辑目标')}</h2>
            <form onSubmit={handleUpdateGoal}>
              ...
              <div className="flex space-x-2 mt-4">
                <button type="submit" className="btn btn-primary flex-1" disabled={isLoading}>
                  {isLoading ? t('goals.updating', '更新中...') : t('goals.updateGoal', '更新目标')}
                </button>
                <button
                  type="button"
        ...
        <div className="goals-list">
          <h2 className="text-xl font-semibold mb-4">{t('goals.myGoals', '我的目标')}</h2>
          {isLoading ? (
            <div className="loading">{t('common.loading', '加载中...')}</div>
          ) : goals.length === 0 ? (
            <div className="empty-state">
              <p>{t('goals.emptyState', '还没有设置目标，点击"添加目标"按钮开始设置吧！')}</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {goals.map((goal) => (
                <div key={goal.id} className="goal-card card">
                  ...
                  <div className="goal-dates mt-4 text-sm text-gray-500">
                    <p>
                      {t('goals.startDateLabel', '开始日期')}：{formatDate(goal.startDate)}
                    </p>
                    <p>
                      {t('goals.endDateLabel', '结束日期')}：{formatDate(goal.endDate)}
                    </p>
                  </div>
                  <div className="goal-actions mt-4 flex space-x-2">
                    {goal.status === 'active' && (
                      ...
                      >
                        {t('goals.incrementProgress', '增加进度')}
                      </button>
                    )}
                    <button
