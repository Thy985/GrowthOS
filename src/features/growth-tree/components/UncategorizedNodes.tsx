    ...
    <div className="mt-6">
      ...
      <div className="space-y-2">
        {uncategorizedNodes.map((node) => (
          <div
            key={node.id}
            className={`flex items-center justify-between tree-node ${node.status}`}
          >
            <span>{node.name}</span>
            <div className="flex space-x-2">
              <button className="btn btn-primary text-xs">确认</button>
              <button className="btn btn-danger text-xs">删除</button>
            </div>
          </div>
        ))}
        <div className="flex items-center justify-between tree-node">
          <span>...</span>
          <button className="btn btn-primary text-xs">查看更多</button>
        </div>
      </div>
