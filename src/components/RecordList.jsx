...
const RecordList = () => {
  ...
  const getFilteredRecords = () => {
    let filtered = searchRecords(searchTerm);
    
    if (selectedMoods.length > 0) {
      filtered = filterRecordsByMood(selectedMoods);
    }
    
    if (selectedTags.length > 0) {
      filtered = filterRecordsByTags(selectedTags);
    }
    
    if (dateRange.start && dateRange.end) {
      filtered = filterRecordsByDateRange(dateRange.start, dateRange.end);
    }
            ...
            <div key={record.id} className="bg-white rounded-lg shadow p-6 hover:shadow-md transition-shadow">
              ...
              {record.learning && (
                <div className="mb-3">
                  ...
                  <p className="text-gray-600">{record.learning}</p>
                </div>
              )}
              ...
              {record.reflection && (
                <div className="mb-3">
                  ...
                  <p className="text-gray-600">{record.reflection}</p>
                </div>
              )}
